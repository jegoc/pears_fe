...
const ManProd = () =>{
    ...
    React.useEffect(()=>{
        const fetchData = async ()=> {
                try{
                    const res = await axios.get(`${apiUrl.url}getprod/${cookie}`);
                    setData(res.data);
                    setFilterd(res.data);
                }catch(err){
                    throw new Error(err);
                    ...
                    <tbody>
                        {data.map((r,i)=> (   
                            <tr key={i}>
                                ...
                                <td>{r.prod_code}</td>
                                <td>{r.prod_desc}</td>
                                <td style={{textAlign:"center"}}>{r.prod_unit}</td>
                                <td style={{textAlign:"right"}}>{r.prod_price}</td>
                                <td style={{textAlign:"center"}}>{r.prod_status===1?'Active':<label style={{color:"red"}}>Inactive</label>}</td>
